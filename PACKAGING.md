...
* 备份文件后缀为 `.bak`, 因此后续扫描不会把 `backup/` 里的备份当作待处理文件;
  关闭备份可用命令行 `--no-backup`, 或在左侧栏关闭 "生成备份文件" 开关
  (默认开启; macOS 为原生 NSSwitch, Windows 为自绘 Win11 开关)。
...
* **侧栏开关**: `NSSwitch` (含子目录 + 生成备份文件, 4 行分组 179 pt);
  **两个下拉**: 原生 `NSPopUpButton` (含 其它… 自定义项, 复用 Qt 输入对话框
  后回填); **应用按钮**: 原生 `NSButton`; **状态栏**: 原生 `NSTextField`;
  **选项复选框**: 原生 `NSButton` (`NSSwitchButton`, 6 个, 覆盖在仅占位的
  Qt spacer 上, 响应式 2×3/3×2 网格仍由 Qt 布局驱动); **窗口标题**: 自绘
  `NSTextField`;
...
右侧为不透明的内容面板 (选项复选框、应用按钮、差异预览与状态栏)。
选项区没有标题, 6 个复选框 (半角标点后加空格 / CJK 与控制序列空格 /
页码范围保持紧凑 / 使用 ~ 代替空格 / 添加编码魔法注释 / 仅检查) 按面板可用
宽度自适应排布: 宽度足够时为 **2 行 × 3 列**, 变窄 (或把侧栏拖宽) 时变为
**3 行 × 2 列**; 各列等宽并均分面板宽度, 复选框在各自列内左对齐
(窗口缩放与拖动分隔条时即时重排)。左侧栏分组自上而下为:
扩展名、输出编码、含子目录 (开关)、生成备份文件 (开关)。

**"使用 ~ 代替空格"** (命令行 `--tie`) 打开后, **每个分隔符**都改为不换行
空格 `~` (带子): 既包括格式化器**插入**的分隔符, 也包括这些边界处**源文件中
已有的空格** (多空格会并为一个 `~`)。适用于 CJK↔拉丁/数字、CJK↔行内公式、
CJK↔控制序列、`\verb` 前后的空格, 以及半角标点的间距, 例如
`广义Fibonacci 行列式` → `广义~Fibonacci~行列式`,
`如 $\Gamma$ 排正体` → `如~$\Gamma$~排正体`,
`无需写标题 \cite{1}` → `无需写标题~\cite{1}`,
`式 (1) 是` → `式~(1)~是`。注意**控制序列之后**的空格保持不变
(`\hfill  应`、`\songti  期` 不改为 `~`), 控制序列之间 (`\TT \hfill`) 也
不变; 而 CJK 分组结尾 `}` 与后续控制序列之间会变 `~`
(`报} \hfill` → `报}~\hfill`)。控制序列与其后的公式/`\verb` 仍保持贴附
(`\cite $x$` 不变, 与非 tie 规则一致); 数学/`\verb`/逐字环境/注释/引号
内部的空格不受影响。
**改动任一选项 (含该开关) 都会立即重新渲染所选文件的预览**, 无需重新选择文件。
...
format-tex --tie file.tex          # 插入的分隔符一律用 ~ 而不是空格
