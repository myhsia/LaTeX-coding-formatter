...
**"使用 ~ 代替空格"** (命令行 `--tie`) 打开后, 格式化器**插入的每个分隔符**
都改为不换行空格 `~` (带子) 而不是普通空格: CJK↔拉丁/数字、CJK↔行内公式、
CJK↔控制序列、`\verb` 之后的空格, 以及半角标点的间距都适用 (例如
`生成~pdf~文件`、`如~$\Gamma$~排正体`、`无需写标题~\cite{1}`、`式~(1)`);
`\verb` 之前的 `~` 本来就始终使用。源文件中已有的空格不受影响。
